...
const IngredientsPage = () => {
  ...
  const emptyForm = { name: '', unit: 'kg', costPerUnit: '', supplierId: '', currentStock: '0', minStock: '0', category: '' };
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      const data = { name: formData.name, unit: formData.unit, costPerUnit: parseFloat(formData.costPerUnit) || 0, supplierId: formData.supplierId || null, currentStock: parseFloat(formData.currentStock) || 0, minStock: parseFloat(formData.minStock) || 0, category: formData.category || '' };
      ...
      {loading ? (
        ...
        <div className="bg-card rounded-xl border overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={7} className="px-4 py-12 text-center text-muted-foreground"><ShoppingBasket className="w-8 h-8 mx-auto mb-2 opacity-30" /><p>Nenhum ingrediente encontrado</p></td></tr>
              ) : filtered.map(item => {
